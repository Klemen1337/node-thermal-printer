...
import {getInterface, InterfaceT} from './interfaces';
...
interface ThermalPrinterInitConfigI {
  interface: string | InterfaceT;
  options: NetworkOptions;
  driver: any;
  type: PrinterTypes;
  width: string | number;
  characterSet: string; // TODO: make more precise
  removeSpecialCharacters: boolean;
  lineCharacter: string;
}

interface ThermalPrinterConfigI {
  options: NetworkOptions;
  width: number;
  characterSet: string; // TODO: make more precise
  removeSpecialCharacters: boolean;
  lineCharacter: string;
  /** This is a string key for Codepages defined in `lib/types/*-config.ts` */
  codePage?: string;
  type?: PrinterTypes;
}
...
export class ThermalPrinter {
  ...
  printer: any = null;
  // printer: PrinterType | null = null;
  types = PrinterTypes;
  config: ThermalPrinterConfigI = null;
  buffer = null;
  Interface = null;
  constructor ( initConfig: Partial<ThermalPrinterInitConfigI>) {
    ...
    this.config = {
      width: typeof initConfig.width === "number" ? initConfig.width : parseInt(initConfig.width) || 48,
  ...
  getWidth(): number {
    return typeof this.config.width === "string" ? parseInt(this.config.width) : this.config.width;
  }
