...
class Star extends PrinterType {
  ...
  printQR(str, settings) {
    ...
    var s = str.length;
    var lsb = Math.trunc( s % 256);
    var msb = Math.trunc( s / 256);
  ...
  printImageBuffer(width, height, data) {
    ...
    for (var i = 0; i < Math.ceil(height / 24); i++) {
          ...
          for (var x = 0; x < 8; x++) {
            if ((i * 24 + y < pixels.length) && (j * 8 + x < pixels[i * 24 + y].length)) {
              ...
              if (pixel.a > 126) { // checking transparency
                var grayscale = Math.trunc( 0.2126 * pixel.r + 0.7152 * pixel.g + 0.0722 * pixel.b);
      ...
      this.append(Buffer.from([0x1b, 0x6b, Math.trunc( imageBuffer.length / 24), 0x00]));
