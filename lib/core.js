...
class ThermalPrinter {
  constructor(initConfig) {
    ...
    this.config = {
      ...
      convertEncoding: initConfig.convertEncoding,
    };

    if(typeof this.config.convertEncoding === 'string') {
      try {
        this.iconv = new Iconv('UTF-8', this.config.convertEncoding);
      } catch(e) {
        this.iconv = null;
      }
    }
  ...
  append(buff) {
    if (typeof buff === "string") {
      if(this.iconv !== null) {
        try {
          this.add(this.iconv.convert(buff));
          return;
        } catch(e) {}
      }
