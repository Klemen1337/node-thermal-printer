var fs = require('fs'),
  ...
  Star = require('./types/star'),
  Epson = require('./types/epson'),
...
class ThermalPrinter {
  constructor(initConfig) {
    this.iface = getInterface(initConfig.interface, initConfig.options);
    this.buffer = null;
    this.config = null;
    this.printer = null;
    ...
    if (initConfig.extraSpecialCharacters) this.printer.config.specialCharacters = mergeObjects(this.printer.config.specialCharacters, initConfig.extraSpecialCharacters);
  ...
  async execute() {
    try {
      return await this.iface.execute(this.buffer);
    } catch (error) {
      throw error;
    }
  }
  ...
  async isPrinterConnected(exists) {
    return await this.iface.isPrinterConnected(exists);
  }
  ...
  async printImageBuffer(buffer) {
    ...
    return new Promise((resolve, reject) => {
      ...
      png.on('parsed', function () {
        that.append(that.printerType._printImageBuffer(this.width, this.height, this.data));
        resolve(response);
      });
  ...
  async raw(text) {
    try {
      return await this.iface.execute(text);
    } catch (error) {
      throw error;
    }
  }
