...
class ThermalPrinter {
  constructor(initConfig) {
    const getInterface = require("./interfaces").default;
  ...
  executeCommand(command) {
    return this.Interface.executeCommand(command, this.config.options && this.config.options.commandTimeout || 150);
  }

  async readPeripheralStatus() {
    return (await this.executeCommand(this.printer.config.DEVICE_STATUS))[0];
  }

  async readPaperStatus() {
    return (await this.executeCommand(this.printer.config.PAPER_STATUS))[0];
  }
}
