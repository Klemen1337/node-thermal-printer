    ...
    try {
      ...
      if (image.slice(-4) === ".png") {
        if (printerConfig.type === printerTypes.STAR) {
          // Start image print
          try {
            let response = await epson.printImageStar(image);
            append(response);
            return response;
          } catch (error) {
            throw error;
          ...
          // Epson image print
          try {
            let response = await epson.printImageEpson(image);
            append(response);
            return response;
          } catch (error) {
            throw error;
  ...
  printImageBuffer: async function (buffer) {
    return new Promise((resolve, reject) => {
      ...
      png.on('parsed', function () {
        if (printerConfig.type === printerTypes.STAR) {
          let response = star._printImageBufferStar(this.width, this.height, this.data)
          resolve(response);
        } else {
          let response = epson._printImageBufferEpson(this.width, this.height, this.data)
          resolve(response);
        }
      });
